...
class TeamsListRepository extends Repository
{
    ...
    public function get_all_teams_and_members_concat(string $role_name): array
    {
        try {
            ...
            GROUP_CONCAT(DISTINCT CASE WHEN users.is_anonymous = 1 THEN 'Participant anonyme' ELSE CONCAT(users.first_name,' ',users.last_name) END SEPARATOR ', ') as members, GROUP_CONCAT(DISTINCT contact_person.name SEPARATOR ', ') as contact_person_name, GROUP_CONCAT(DISTINCT contact_person.email SEPARATOR ', ') as contact_person_email
            FROM users 
            INNER JOIN users_teams ON users.id = users_teams.users_id 
            INNER JOIN teams ON teams.id = users_teams.teams_id 
            INNER JOIN categories ON teams.categories_id = categories.id 
    ...
    {
        try {
            $sql = "SELECT users.id, teams.id as team_id, teams.team_number, teams.name as title, teams.description, categories.name as category, teams.years as year, evaluationgrids.name as survey, survey.id as survey_id, teams.activated as teams_activated, 
            users.first_name, users.last_name, users.numero_da, users.email, users.picture_consent, users.picture_consent_scope, users.is_anonymous, users.activated as users_activated, users.blacklisted, contact_person.name as contact_name, contact_person.email as contact_email, contact_person.id as contact_id
            FROM users 
            INNER JOIN users_teams ON users.id = users_teams.users_id 
            INNER JOIN teams ON teams.id = users_teams.teams_id 
            INNER JOIN categories ON teams.categories_id = categories.id 
